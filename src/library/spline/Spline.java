...
public class Spline {
 ...
 public SplinePoint getSpline(float t) {
    ...
    // Bounds checking
    p0 = Math.max(0, Math.min(p0, points.length - 1));
    p1 = Math.max(0, Math.min(p1, points.length - 1));
    p2 = Math.max(0, Math.min(p2, points.length - 1));
    p3 = Math.max(0, Math.min(p3, points.length - 1));
